...
class GFStripe extends GFPaymentAddOn {
	...
	protected $_slug = 'gf-stripe-integration';
	protected $_path = 'gf-stripe-integration/stripe.php';
		...
		$scripts = array(
				...
				array(
						...
						'strings'   => array(
								...
								'validation_error' => esc_html__( 'Error validating this key. Please try again later.', 'gf-stripe-integration' ),
	...
	public function plugin_settings_fields() {
		return array(
			array(
				'title'  => esc_html__( 'Stripe API', 'gf-stripe-integration' ),
				'fields' => $this->api_settings_fields()
			),
			array(
				'title'       => esc_html__( 'Stripe Webhooks', 'gf-stripe-integration' ),
				'description' => $this->get_webhooks_section_description(),
				'fields'      => array(
					array(
						'name'       => 'webhooks_enabled',
						'label'      => esc_html__( 'Webhooks Enabled?', 'gf-stripe-integration' ),
						'type'       => 'checkbox',
						'horizontal' => true,
						'required'   => 1,
						'choices'    => array(
							array(
								'label' => sprintf( esc_html__( 'I have enabled the Gravity Forms webhook URL in my Stripe account.', 'gf-stripe-integration' ) ),
								'value' => 1,
								'name'  => 'webhooks_enabled',
							),
						)
					...
					array(
						'type'     => 'save',
						'messages' => array( 'success' => esc_html__( 'Settings updated successfully', 'gf-stripe-integration' ) )
	...
	public function api_settings_fields() {
		return array(
				array(
						'name'          => 'api_mode',
						'label'         => esc_html__( 'API', 'gf-stripe-integration' ),
						'type'          => 'radio',
						'default_value' => 'live',
						'choices'       => array(
								array(
										'label' => esc_html__( 'Live', 'gf-stripe-integration' ),
										'value' => 'live',
								),
								array(
										'label'    => esc_html__( 'Test', 'gf-stripe-integration' ),
										'value'    => 'test',
										'selected' => true,
								),
						),
				...
				array(
						...
						'label'    => esc_html__( 'Test Secret Key', 'gf-stripe-integration' ),
						'type'     => 'text',
						'class'    => 'medium',
				...
				array(
						...
						'label'    => esc_html__( 'Test Publishable Key', 'gf-stripe-integration' ),
						'type'     => 'text',
						'class'    => 'medium',
				...
				array(
						...
						'label'    => esc_html__( 'Live Secret Key', 'gf-stripe-integration' ),
						'type'     => 'text',
						'class'    => 'medium',
				...
				array(
						...
						'label'    => esc_html__( 'Live Publishable Key', 'gf-stripe-integration' ),
						'type'     => 'text',
						'class'    => 'medium',
	...
	public function get_webhooks_section_description() {
		...
		<?php esc_html_e( 'Gravity Forms requires the following URL to be added to your Stripe account\'s list of Webhooks.', 'gf-stripe-integration' ); ?>
		<a href="javascript:return false;"
		   onclick="jQuery('#stripe-webhooks-instructions').slideToggle();"><?php esc_html_e( 'View Instructions', 'gf-stripe-integration' ); ?></a>

		<div id="stripe-webhooks-instructions" style="display:none;">

			<ol>
				<li>
					<?php esc_html_e( 'Click the following link and log in to access your Stripe Webhooks management page:', 'gf-stripe-integration' ); ?>
					<br/>
					<a href="https://dashboard.stripe.com/account/webhooks" target="_blank">https://dashboard.stripe.com/account/webhooks</a>
				</li>
				<li><?php esc_html_e( 'Click the "Add Endpoint" button above the list of Webhook URLs.', 'gf-stripe-integration' ); ?></li>
				<li>
					<?php esc_html_e( 'Enter the following URL in the "URL" field:', 'gf-stripe-integration' ); ?>
					<code><?php echo $this->get_webhook_url(); ?></code>
				</li>
				<li><?php esc_html_e( 'Select "Live" from the "Mode" drop down.', 'gf-stripe-integration' ); ?></li>
				<li><?php esc_html_e( 'Click the "Create Endpoint" button.', 'gf-stripe-integration' ); ?></li>
			</ol>
	...
	public function feed_settings_fields() {
		...
		$customer_info_field = array(
				...
				'label'      => esc_html__( 'Customer Information', 'gf-stripe-integration' ),
				'type'       => 'field_map',
				'dependency' => array(
				...
				'field_map'  => array(
						array(
								'name'       => 'email',
								'label'      => esc_html__( 'Email', 'gf-stripe-integration' ),
								'required'   => true,
								'field_type' => array(
										'email',
										'hidden',
						...
						array(
								'name'     => 'description',
								'label'    => esc_html__( 'Description', 'gf-stripe-integration' ),
								'required' => false,
						),
						array(
								'name'       => 'coupon',
								'label'      => esc_html__( 'Coupon', 'gf-stripe-integration' ),
								'required'   => false,
								'field_type' => array(
										'coupon',
										'text',
								),
								'tooltip' => '<h6>' . esc_html__( 'Coupon', 'gf-stripe-integration' ) . '</h6>' . esc_html__( 'Select which field contains the coupon code to be applied to the recurring charge(s). The coupon must also exist in your Stripe Dashboard.', 'gf-stripe-integration' ),
						),
				)
		);
		...
		if ( $this->get_setting( 'transactionType' ) == 'subscription' ) {
			$info = esc_html__( 'You will see this data when viewing a customer page.', 'gf-stripe-integration' );
		} else {
			$info = esc_html__( 'You will see this data when viewing a payment page.', 'gf-stripe-integration' );
		}
		//add custom  meta information
		$custom_meta = array(
			array(
				'name'                => 'metaData',
				'label'               => esc_html__( 'Metadata', 'gf-stripe-integration' ),
				'type'                => 'dynamic_field_map',
				'limit'				  => 20,
				'exclude_field_types' => 'creditcard',
				'tooltip'             => '<h6>' . esc_html__( 'Metadata', 'gf-stripe-integration' ) . '</h6>' . esc_html__( 'You may send custom meta information to Stripe. A maximum of 20 custom keys may be sent. The key name must be 40 characters or less, and the mapped data will be truncated to 500 characters per requirements by Stripe. ' . $info , 'gf-stripe-integration' ),
		...
		$trial_period_field = array(
			'name'                => 'trialPeriod',
			'label'               => esc_html__( 'Trial Period', 'gf-stripe-integration' ),
		...
		if ( $this->get_setting( 'transactionType' ) == 'product' ) {
			$receipt_settings = array(
				...
				'type'    => 'receipt',
				'tooltip' => '<h6>' . esc_html__( 'Stripe Receipt', 'gf-stripe-integration' ) . '</h6>' . esc_html__( 'Stripe can send a receipt via email upon payment. Select an email field to enable this feature.', 'gf-stripe-integration' )
			);
	...
	public function settings_receipt( $field, $echo = true ) {
		...
		$first_choice = array( 'label' => esc_html__( 'Do not send receipt', 'gf-stripe-integration' ), 'value' => '' );
	...
	public function settings_setup_fee( $field, $echo = true ) {
		...
		$enabled_field = array(
			...
			'choices'    => array(
				array(
					'label'    => esc_html__( 'Enabled', 'gf-stripe-integration' ),
	...
	public function settings_trial_period( $field, $echo = true ) {
		...
		$html .= ' <span class="gaddon-settings-input-suffix">' . esc_html__( 'days', 'gf-stripe-integration' ) . '</span>';
	...
	public function validate_trial_period( $field ) {
		...
		if ( $settings['trial_enabled'] && ( empty( $settings['trialPeriod'] ) || ! ctype_digit( $settings['trialPeriod'] ) ) ) {
			$this->set_field_error( array( 'name' => 'trialValidationPlaceholder' ), esc_html__( 'Please enter a valid number of days.', 'gf-stripe-integration' ) );
		}
	...
	public function validate_custom_meta( $field ) {
		...
		if ( $metaCount > 20 ) {
			$this->set_field_error( array( esc_html__( 'You may only have 20 custom keys.' ), 'gf-stripe-integration' ) );
		...
		foreach ( $metaData as $meta ) {
			if ( empty( $meta['custom_key'] ) && ! empty( $meta['value'] ) ) {
				$this->set_field_error( array( 'name' => 'metaData' ), esc_html__( "A field has been mapped to a custom key without a name. Please enter a name for the custom key, remove the metadata item, or return the corresponding drop down to 'Select a Field'.", 'gf-stripe-integration' ) );
				break;
			} elseif ( strlen( $meta['custom_key'] ) > 40 ) {
				$this->set_field_error( array( 'name' => 'metaData' ), sprintf( esc_html__( 'The name of custom key %s is too long. Please shorten this to 40 characters or less.', 'gf-stripe-integration' ), $meta['custom_key'] ) );
				break;
			}
		}
	}
	...
	public function supported_billing_intervals() {
		...
		$supported_billing_cycles = array(
			'day'   => array( 'label' => esc_html__( 'day(s)', 'gf-stripe-integration' ), 'min' => 1, 'max' => 365 ),
			'week'  => array( 'label' => esc_html__( 'week(s)', 'gf-stripe-integration' ), 'min' => 1, 'max' => 12 ),
			'month' => array( 'label' => esc_html__( 'month(s)', 'gf-stripe-integration' ), 'min' => 1, 'max' => 12 ),
			'year'  => array( 'label' => esc_html__( 'year(s)', 'gf-stripe-integration' ), 'min' => 1, 'max' => 1 )
		);
	...
	public function supported_notification_events( $form ) {
		...
		return array(
				'complete_payment'          => esc_html__( 'Payment Completed', 'gf-stripe-integration' ),
				'refund_payment'            => esc_html__( 'Payment Refunded', 'gf-stripe-integration' ),
				'fail_payment'              => esc_html__( 'Payment Failed', 'gf-stripe-integration' ),
				'create_subscription'       => esc_html__( 'Subscription Created', 'gf-stripe-integration' ),
				'cancel_subscription'       => esc_html__( 'Subscription Canceled', 'gf-stripe-integration' ),
				'add_subscription_payment'  => esc_html__( 'Subscription Payment Added', 'gf-stripe-integration' ),
				'fail_subscription_payment' => esc_html__( 'Subscription Payment Failed', 'gf-stripe-integration' ),
		);
	}
	...
	public function callback() {
		...
		if ( $response['id'] == 'evt_00000000000000' ) {
			return new WP_Error( 'test_webhook_succeeded', __( 'Test webhook succeeded. Your Stripe Account and Stripe Add-On are configured correctly to process webhooks.', 'gf-stripe-integration' ), array( 'status_header' => 200 ) );
		}
		...
		if ( $response['livemode'] == false && $mode == 'live' ) {
			return new WP_Error( 'invalid_request', __( 'Webhook from test transaction. Bypassed.', 'gf-stripe-integration' ) );
		}
		...
		try {
			...
			return new WP_Error( 'invalid_request', __( 'Invalid webhook data. Webhook could not be processed.', 'gf-stripe-integration' ), array( 'status_header' => 500 ) );
		}
		...
		switch ( $type ) {
			...
			case 'charge.refunded':
				...
				if ( ! $entry_id ) {
					return new WP_Error( 'entry_not_found', sprintf( __( 'Entry for transaction id: %s was not found. Webhook cannot be processed.', 'gf-stripe-integration' ), $action['transaction_id'] ) );
				}
			...
			case 'customer.subscription.deleted':
				...
				if ( ! $entry_id ) {
					return new WP_Error( 'entry_not_found', sprintf( __( 'Entry for subscription id: %s was not found. Webhook cannot be processed.', 'gf-stripe-integration' ), $action['subscription_id'] ) );
				}
			...
			case 'invoice.payment_succeeded':
				...
				if ( ! $subscription ) {
					return new WP_Error( 'invalid_request', sprintf( __( 'Subscription line item not found in request', 'gf-stripe-integration' ) ) );
				}
				...
				$action['note'] .= sprintf( __( 'Subscription payment has been paid. Amount: %s. Subscriber Id: %s', 'gf-stripe-integration' ), $amount_formatted, $action['subscription_id'] );
	...
	public function get_captured_payment_note( $entry_id ) {
		...
		if ( rgars( $feed, 'meta/setupFee_enabled' ) ) {
			$note = esc_html__( 'Setup fee has been paid.', 'gf-stripe-integration' );
		} else {
			$note = esc_html__( 'Trial has been paid.', 'gf-stripe-integration' );
		}
	...
	public function get_stripe_js_error() {
		...
		if ( isset( $response->error ) ) {
			...
			return esc_html__( 'Unable to authorize card. No response from Stripe.js.', 'gf-stripe-integration' );
		}
	...
	public function get_payment_description( $entry, $submission_data, $feed ) {
		...
		if ( $entry['id'] ) {
			$strings['entry_id'] = sprintf( esc_html__( 'Entry ID: %d', 'gf-stripe-integration' ), $entry['id'] );
		}

		$strings['products'] = sprintf(
			_n( 'Product: %s', 'Products: %s', count( $submission_data['line_items'] ), 'gf-stripe-integration' ),
	...
	public function is_field_on_valid_page( $field, $parent ) {
		...
		if ( $mapped_field_page > $cc_page ) {
			$this->set_field_error( $field, esc_html__( 'The selected field needs to be on the same page as the Credit Card field or a previous page.', 'gf-stripe-integration' ) );
		}
